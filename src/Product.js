...
const Product = () => {
  ...
  useEffect(() => {
    axios.get('https://backend-iuq5.vercel.app/api')
      .then(res => {
  ...
  const handleDelete = async (id) => {
    try {
        ...
        const response = await axios.delete(`https://backend-iuq5.vercel.app/api/product/${id}`);
  ...
  const handleSave = async () => {
    ...
    try {
        const response = await axios.put(`https://backend-iuq5.vercel.app/api/update/${editedProduct['S.no']}`, updatedProduct);
