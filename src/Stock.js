...
function Stock() {
  ...
  // Include statusFilter in dependencies       
  ...
  const fetchStocks = async (dept, location, statusFilter) => {
    try {
        const params = { department: dept, status: statusFilter || 'collected' };
        
        if (dept && dept.toLowerCase() === 'management') {
            // Only pass status if department is 'Management'
  ...
  const handleStatusChange = (id, status, type, quantity) => {
    ...
    axios.put(`https://backend-iuq5.vercel.app/api/updateStatus/${id}`, { status: newStatus, type, quantity })
      .then(res => {
